...
bool Hdf5FieldExtraction_vz_f::validate_data_type()
{
	...
	for (size_t mem_id = 0; mem_id < mem_num; ++mem_id)
	{
		...
		if (strcmp(mem_name, "vz_f") == 0)
		{
			vz_f_offset = H5Tget_member_offset(pcl_dt_id, mem_id);
			vz_f_fld_is_found = true;
			break;
		}
	}
	return vz_f_fld_is_found;
}

int Hdf5FieldExtraction_vz_f::extract_pcl_fld_data(double* pcl_fld_data)
{
	if (!vz_f_fld_is_found)
		return -1;
	...
	{
		pcl_fld_data[pcl_id] = *(double *)(cur_pcl + vz_f_offset);
		cur_pcl += pcl_size;
	}
	return 0;
}

...
bool Hdf5FieldExtraction_vy_s::validate_data_type()
{
	...
	for (size_t mem_id = 0; mem_id < mem_num; ++mem_id)
	{
		...
		if (strcmp(mem_name, "vy_s") == 0)
		{
			vy_s_offset = H5Tget_member_offset(pcl_dt_id, mem_id);
			vy_s_fld_is_found = true;
			break;
		}
	}
	return vy_s_fld_is_found;
}

int Hdf5FieldExtraction_vy_s::extract_pcl_fld_data(double* pcl_fld_data)
{
	if (!vy_s_fld_is_found)
		return -1;
	...
	{
		pcl_fld_data[pcl_id] = *(double *)(cur_pcl + vy_s_offset);
		cur_pcl += pcl_size;
	}
	return 0;
}

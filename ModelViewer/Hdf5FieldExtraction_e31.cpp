...
bool Hdf5FieldExtraction_e31::validate_data_type()
{
	...
	for (size_t mem_id = 0; mem_id < mem_num; ++mem_id)
	{
		...
		if (strcmp(mem_name, "e31") == 0)
		{
			e31_offset = H5Tget_member_offset(pcl_dt_id, mem_id);
			e31_fld_is_found = true;
			break;
		}
	}
	return e31_fld_is_found;
}

int Hdf5FieldExtraction_e31::extract_pcl_fld_data(double* pcl_fld_data)
{
	if (!e31_fld_is_found)
		return -1;
	...
	{
		pcl_fld_data[pcl_id] = *(double *)(cur_pcl + e31_offset);
		cur_pcl += pcl_size;
	}
	return 0;
}

...
QtApp_Posp_T3D_CHM_s::QtApp_Posp_T3D_CHM_s(int &argc, char **argv, Type tp) :
	app(argc, argv), type(tp), scene(window.get_view()),
	pcontroller(nullptr)
{
	switch (tp)
	{
...
QtApp_Posp_T3D_CHM_s::~QtApp_Posp_T3D_CHM_s()
{
	if (pcontroller)
	{
		delete pcontroller;
...
int QtApp_Posp_T3D_CHM_s::start()
{
	window.show();
	return app.exec();
}

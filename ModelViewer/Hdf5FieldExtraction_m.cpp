...
bool Hdf5FieldExtraction_m::validate_data_type()
{
	...
	return m_fld_is_found;
}

int Hdf5FieldExtraction_m::extract_pcl_fld_data(double* pcl_fld_data)
{
	if (!m_fld_is_found)
		return -1;
	...
	{
		pcl_fld_data[pcl_id] = *(double *)(cur_pcl + m_offset);
		cur_pcl += pcl_size;
	}
	return 0;
}

int Hdf5FieldExtraction_m::extract_pcl_fld_data_f(float *pcl_fld_data)
{
	if (!m_fld_is_found)
		return -1;
	...
	for (size_t pcl_id = 0; pcl_id < pcl_num; ++pcl_id)
	{
		pcl_fld_data[pcl_id] = *(float *)(cur_pcl + m_offset);
		cur_pcl += pcl_size;
	}
	return 0;
}

...
bool Hdf5FieldExtraction_y::validate_data_type()
{
...
int Hdf5FieldExtraction_y::extract_pcl_fld_data(double* pcl_fld_data)
{
	if (!y_fld_is_found)
		return -1;
	...
	{
		pcl_fld_data[pcl_id] = *(double *)(cur_pcl + y_offset);
		cur_pcl += pcl_size;
	}
	return 0;
}

int Hdf5FieldExtraction_y::extract_pcl_fld_data_f(float* pcl_fld_data)
{
	if (!y_fld_is_found)
		return -1;
	...
	for (size_t pcl_id = 0; pcl_id < pcl_num; ++pcl_id)
	{
		pcl_fld_data[pcl_id] = *(float*)(cur_pcl + y_offset);
		cur_pcl += pcl_size;
	}
	return 0;
}

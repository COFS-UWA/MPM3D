...
bool Hdf5FieldExtraction_m_f::validate_data_type()
{
	...
	for (size_t mem_id = 0; mem_id < mem_num; ++mem_id)
	{
		...
		else if (strcmp(mem_name, "density_f") == 0)
		{
...
int Hdf5FieldExtraction_m_f::extract_pcl_fld_data(double* pcl_fld_data)
{
	if (!all_fld_is_found)
		return -1;
	...
	double pcl_m_s, pcl_density_s;
	double pcl_n, pcl_density_f;
	for (size_t pcl_id = 0; pcl_id < pcl_num; ++pcl_id)
	{
		pcl_m_s = *(double *)(cur_pcl + m_s_offset);
		pcl_density_s = *(double *)(cur_pcl + density_s_offset);
		pcl_n = *(double *)(cur_pcl + n_offset);
		pcl_density_f = *(double *)(cur_pcl + density_f_offset);
		pcl_fld_data[pcl_id] = pcl_m_s / ((1.0 - pcl_n) * pcl_density_s)
							 * pcl_n * pcl_density_f;
		cur_pcl += pcl_size;
	}
	return 0;

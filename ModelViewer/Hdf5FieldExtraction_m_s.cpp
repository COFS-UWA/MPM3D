...
bool Hdf5FieldExtraction_m_s::validate_data_type()
{
	...
	for (size_t mem_id = 0; mem_id < mem_num; ++mem_id)
	{
		...
		if (strcmp(mem_name, "x") == 0)
		{
			...
			m_s_fld_is_found = true;
			break;
		}
	}
	return m_s_fld_is_found;
}

int Hdf5FieldExtraction_m_s::extract_pcl_fld_data(double* pcl_fld_data)
{
	if (!m_s_fld_is_found)
		return -1;
	...
	{
		pcl_fld_data[pcl_id] = *(double *)(cur_pcl + m_s_offset);
		cur_pcl += pcl_size;
	}
	return 0;
}

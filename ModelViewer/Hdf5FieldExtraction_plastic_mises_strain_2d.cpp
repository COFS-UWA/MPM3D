...
bool Hdf5FieldExtraction_plastic_mises_strain_2d::validate_data_type()
{
	...
	for (size_t mem_id = 0; mem_id < mem_num; ++mem_id)
	{
		...
		else if (strcmp(mem_name, "pe22") == 0)
		{
			pe22_offset = H5Tget_member_offset(pcl_dt_id, mem_id);
			++tag;
		}
		else if (strcmp(mem_name, "pe12") == 0)
		{
			pe12_offset = H5Tget_member_offset(pcl_dt_id, mem_id);
			++tag;
		}
		if (tag == 3)
		{
...
int Hdf5FieldExtraction_plastic_mises_strain_2d::extract_pcl_fld_data(double* pcl_fld_data)
{
	if (!all_fld_is_found)
		return -1;
	...
	{
		pcl_e11 = *(double *)(cur_pcl + pe11_offset);
		pcl_e22 = *(double *)(cur_pcl + pe22_offset);
		pcl_e12 = *(double *)(cur_pcl + pe12_offset);

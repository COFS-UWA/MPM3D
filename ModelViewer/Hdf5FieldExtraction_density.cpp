...
bool Hdf5FieldExtraction_density::validate_data_type()
{
	...
	for (size_t mem_id = 0; mem_id < mem_num; ++mem_id)
	{
		...
		if (strcmp(mem_name, "x") == 0)
		{
			...
			density_fld_is_found = true;
			break;
		}
	}
	return density_fld_is_found;
}

int Hdf5FieldExtraction_density::extract_pcl_fld_data(double* pcl_fld_data)
{
	if (!density_fld_is_found)
		return -1;
	...
	{
		pcl_fld_data[pcl_id] = *(double *)(cur_pcl + density_offset);
		cur_pcl += pcl_size;
	}
	return 0;
}

int Hdf5FieldExtraction_density::extract_pcl_fld_data_f(float *pcl_fld_data)
{
	if (!density_fld_is_found)
		return -1;
	...
	for (size_t pcl_id = 0; pcl_id < pcl_num; ++pcl_id)
	{
		pcl_fld_data[pcl_id] = *(float *)(cur_pcl + density_offset);
		cur_pcl += pcl_size;
	}
	return 0;
}

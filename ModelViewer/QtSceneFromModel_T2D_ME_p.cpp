...
#include "QtSceneFromModel_T2D_ME_p.h"

QtSceneFromModel_T2D_ME_p::QtSceneFromModel_T2D_ME_p(
...
void QtSceneFromModel_T2D_ME_p::set_viewport(
...
int QtSceneFromModel_T2D_ME_p::initialize(int wd, int ht)
{
	// init shaders
	// shader_plain2D
...
void QtSceneFromModel_T2D_ME_p::draw()
{
...
void QtSceneFromModel_T2D_ME_p::resize(int wd, int ht)
{
	set_viewport(wd, ht, xu - xl, yu - yl);
}

int QtSceneFromModel_T2D_ME_p::set_pts_from_pcl_id(
	size_t* ids, size_t id_num, float radius)
{
	...
	{
		Model_T2D_ME_p::Particle& pcl = pcls[ids[p_id]];
...
int QtSceneFromModel_T2D_ME_p::set_pts_from_node_id(
	size_t* ids, size_t id_num, float radius)
{
	if (!model || !ids || !id_num || radius <= 0.0f)
		return -1;
	...
	pts_mem.reserve(id_num);
	pts = pts_mem.get_mem();
	Model_T2D_ME_p::Node* nodes = model->get_nodes();
	for (size_t p_id = 0; p_id < id_num; ++p_id)
	{
		Model_T2D_ME_p::Node& n = nodes[ids[p_id]];
		PointData& pd = pts[p_id];
		pd.x = GLfloat(n.x);
		pd.y = GLfloat(n.y);
	}

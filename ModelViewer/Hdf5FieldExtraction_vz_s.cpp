...
bool Hdf5FieldExtraction_vz_s::validate_data_type()
{
	...
	for (size_t mem_id = 0; mem_id < mem_num; ++mem_id)
	{
		...
		if (strcmp(mem_name, "vz_s") == 0)
		{
			vz_s_offset = H5Tget_member_offset(pcl_dt_id, mem_id);
			vz_s_fld_is_found = true;
			break;
		}
	}
	return vz_s_fld_is_found;
}

int Hdf5FieldExtraction_vz_s::extract_pcl_fld_data(double* pcl_fld_data)
{
	if (!vz_s_fld_is_found)
		return -1;
	...
	{
		pcl_fld_data[pcl_id] = *(double *)(cur_pcl + vz_s_offset);
		cur_pcl += pcl_size;
	}
	return 0;
}

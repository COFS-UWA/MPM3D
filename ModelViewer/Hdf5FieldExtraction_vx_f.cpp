...
bool Hdf5FieldExtraction_vx_f::validate_data_type()
{
	...
	for (size_t mem_id = 0; mem_id < mem_num; ++mem_id)
	{
		...
		if (strcmp(mem_name, "vx_f") == 0)
		{
			vx_f_offset = H5Tget_member_offset(pcl_dt_id, mem_id);
			vx_f_fld_is_found = true;
			break;
		}
	}
	return vx_f_fld_is_found;
}

int Hdf5FieldExtraction_vx_f::extract_pcl_fld_data(double* pcl_fld_data)
{
	if (!vx_f_fld_is_found)
		return -1;
	...
	{
		pcl_fld_data[pcl_id] = *(double *)(cur_pcl + vx_f_offset);
		cur_pcl += pcl_size;
	}
	return 0;
}

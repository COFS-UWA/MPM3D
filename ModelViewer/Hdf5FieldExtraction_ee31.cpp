...
bool Hdf5FieldExtraction_ee31::validate_data_type()
{
	...
	for (size_t mem_id = 0; mem_id < mem_num; ++mem_id)
	{
		...
		if (strcmp(mem_name, "ee31") == 0)
		{
			ee31_offset = H5Tget_member_offset(pcl_dt_id, mem_id);
			ee31_fld_is_found = true;
			break;
		}
	}
	return ee31_fld_is_found;
}

int Hdf5FieldExtraction_ee31::extract_pcl_fld_data(double* pcl_fld_data)
{
	if (!ee31_fld_is_found)
		return -1;
	...
	{
		pcl_fld_data[pcl_id] = *(double *)(cur_pcl + ee31_offset);
		cur_pcl += pcl_size;
	}
	return 0;
}

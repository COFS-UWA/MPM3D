...
QtSceneFromModel_T3D_CHM_s::QtSceneFromModel_T3D_CHM_s(
...
void QtSceneFromModel_T3D_CHM_s::update_view_mat()
{
...
void QtSceneFromModel_T3D_CHM_s::update_proj_mat()
{
...
void QtSceneFromModel_T3D_CHM_s::update_light_pos()
{
...
int QtSceneFromModel_T3D_CHM_s::set_pts_from_pcl_id(
	size_t* ids, size_t id_num, float radius)
{
	...
	{
		Model_T3D_CHM_s::Particle& pcl = pcls[ids[p_id]];
...
int QtSceneFromModel_T3D_CHM_s::set_pts_from_node_id(
	size_t* ids, size_t id_num, float radius)
{
	if (!model || !ids || !id_num || radius <= 0.0f)
		return -1;
	...
	pts_mem.reserve(id_num);
	pts = pts_mem.get_mem();
	Model_T3D_CHM_s::Node* nodes = model->get_nodes();
	for (size_t p_id = 0; p_id < id_num; ++p_id)
	{
		Model_T3D_CHM_s::Node& n = nodes[ids[p_id]];
		PointData& pd = pts[p_id];
		pd.x = GLfloat(n.x);
		pd.y = GLfloat(n.y);
		pd.z = GLfloat(n.z);
...
int QtSceneFromModel_T3D_CHM_s::initialize(int wd, int ht)
{
...
void QtSceneFromModel_T3D_CHM_s::draw()
{
...
void QtSceneFromModel_T3D_CHM_s::resize(int wd, int ht)
{
	width = wd;
	height = ht;

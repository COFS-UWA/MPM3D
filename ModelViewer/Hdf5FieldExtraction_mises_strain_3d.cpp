...
#include "Hdf5FieldExtraction_mises_strain_3d.h"

bool Hdf5FieldExtraction_mises_strain_3d::validate_data_type()
{
	...
	for (size_t mem_id = 0; mem_id < mem_num; ++mem_id)
	{
		...
		else if (strcmp(mem_name, "e23") == 0)
		{
			e23_offset = H5Tget_member_offset(pcl_dt_id, mem_id);
			++tag;
		}
		else if (strcmp(mem_name, "e31") == 0)
		{
			e31_offset = H5Tget_member_offset(pcl_dt_id, mem_id);
			++tag;
		}
		if (tag == 6)
		{
...
int Hdf5FieldExtraction_mises_strain_3d::extract_pcl_fld_data(double* pcl_fld_data)
{
	if (!all_fld_is_found)
		return -1;
	...
	double pcl_e11, pcl_e22, pcl_e33;
	double pcl_e12, pcl_e23, pcl_e31;
	for (size_t pcl_id = 0; pcl_id < pcl_num; ++pcl_id)
	{
		pcl_e11 = *(double *)(cur_pcl + e11_offset);
		pcl_e22 = *(double *)(cur_pcl + e22_offset);
		pcl_e33 = *(double *)(cur_pcl + e33_offset);
		pcl_e12 = *(double *)(cur_pcl + e12_offset);
		pcl_e23 = *(double *)(cur_pcl + e23_offset);
		pcl_e31 = *(double *)(cur_pcl + e31_offset);
		pcl_fld_data[pcl_id] = 2.0 / 3.0 * sqrt(0.5 *
			 ((pcl_e11 - pcl_e22) * (pcl_e11 - pcl_e22)
			+ (pcl_e22 - pcl_e33) * (pcl_e22 - pcl_e33)
			+ (pcl_e33 - pcl_e11) * (pcl_e33 - pcl_e11)
			+ 6.0 * (pcl_e12 * pcl_e12 + pcl_e23 * pcl_e23 + pcl_e31 * pcl_e31)));
		cur_pcl += pcl_size;
	}
	return 0;
}

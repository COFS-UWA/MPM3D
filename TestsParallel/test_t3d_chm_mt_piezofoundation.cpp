...
void test_T3D_CHM_mt_piezofoundation_model(int argc, char** argv)
{
	...
	teh_mesh.clear();
	model.init_pcls(pcl_generator, 0.3, den_grain, 1000.0, 1.0e4, 1.0e-5, 1.0);
	pcl_generator.clear();
	...
	model.set_contact_param(20000.0 / (sml_pcl_size * sml_pcl_size),
		...
		200.0 / (sml_pcl_size * sml_pcl_size),
		200.0 / (sml_pcl_size * sml_pcl_size));
	...
	//md_disp.set_pts_from_vx_bc_s(0.05);
	//md_disp.set_pts_from_vy_bc_s(0.05);
	//md_disp.set_pts_from_vz_bc_s(0.05);
	//md_disp.set_pts_from_vx_bc_f(0.05);
	//md_disp.set_pts_from_vy_bc_f(0.05);
	md_disp.set_pts_from_vz_bc_f(0.05);
	md_disp.start();
}

void test_T3D_CHM_mt_piezofoundation_geo(int argc, char** argv)
{
	Model_T3D_CHM_mt model;
	Model_T3D_CHM_mt_hdf5_utilities::load_model_from_hdf5_file(
		model, "t3d_chm_mt_piezofoundation_model.h5");
	...
	ResultFile_hdf5 res_file_hdf5;
	res_file_hdf5.create("t3d_chm_mt_piezofoundation_geo.h5");
...
void test_T3D_CHM_mt_piezofoundation(int argc, char** argv)
{
	Model_T3D_CHM_mt model;
	Step_T3D_CHM_mt step("step2");
	Model_T3D_CHM_mt_hdf5_utilities::load_model_from_hdf5_file(
		model, step, "t3d_chm_mt_piezofoundation_geo.h5", "geostatic", 21); // 21
	...
	ResultFile_hdf5 res_file_hdf5;
	res_file_hdf5.create("t3d_chm_mt_piezofoundation.h5");
...
void test_t3d_chm_mt_piezofoundation_geo_result(int argc, char** argv)
{
	ResultFile_hdf5 rf;
	rf.open("t3d_chm_mt_piezofoundation_geo.h5");
	...
	//
	app.set_color_map_geometry(1.2f, 0.4f, 0.45f);
	//app.set_png_name("t3d_chm_mt_piezofoundation_geo");
	app.set_gif_name("t3d_chm_mt_piezofoundation_geo");
	app.start();
}

void test_t3d_chm_mt_piezofoundation_result(int argc, char** argv)
{
	ResultFile_hdf5 rf;
	rf.open("t3d_chm_mt_piezofoundation.h5");
	...
	//
	app.set_color_map_geometry(1.2f, 0.4f, 0.45f);
	app.set_png_name("t3d_chm_mt_piezofoundation");
	app.set_gif_name("t3d_chm_mt_piezofoundation");
	app.start();
}

...
void test_t2d_chm_tbb_1d_consolidation(int argc, char** argv)
{
	...
	res_file_hdf5.create("t2d_chm_tbb_1d_consolidation.h5");

	ModelData_T2D_CHM_mt md;
	md.output_model(model, res_file_hdf5);

	TimeHistory_T2D_CHM_TBB_complete out("consolidation");
	out.set_res_file(res_file_hdf5);
	out.set_interval_num(100);
	out.set_output_init_state();
	out.set_output_final_state();
	TimeHistory_ConsoleProgressBar out_pb;
	out_pb.set_interval_num(200);

	Step_T2D_CHM_TBB step("step1");
...
{
	ResultFile_hdf5 rf;
	rf.open("t2d_chm_tbb_1d_consolidation.h5");
	...
	//app.set_png_name("t2d_chm_tbb_1d_consolidation");
	app.set_gif_name("t2d_chm_tbb_1d_consolidation");
	app.start();
}

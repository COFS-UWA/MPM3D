...
{
	ModelData_T3D_CHM_s &md = static_cast<ModelData_T3D_CHM_s &>(_self);
	Model_T3D_CHM_s &model = static_cast<Model_T3D_CHM_s &>(*md.model);
	...
	Model_T3D_CHM_s::Element *elems = model.get_elems();
	...
	const char *mps_info = ""
		"    <MaterialPoints type = \"CHM_3D\" num=%zu>\n"
		"    </MaterialPoints>\n";
	snprintf(str_buffer, sizeof(str_buffer) / sizeof(str_buffer[0]),
...
int model_data_output_func_t2d_chm_s_to_hdf5_res_file(ModelData &_self)
{
	...
	Model_T3D_CHM_s_hdf5_utilities::output_model_to_hdf5_file(model, rf);
	return 0;
}

#ifndef __Model_T2D_CHM_p_hdf5_utilities_h__
#define __Model_T2D_CHM_p_hdf5_utilities_h__
...
struct ParticleData
{
	...
	double e22;
	double e12;
	void from_pcl(Model_T2D_CHM_p::Particle &pcl)
	{
		id = pcl.id;
		m_s = pcl.m_s;
	...
	void to_pcl(Model_T2D_CHM_p::Particle &pcl)
	{
		pcl.id = id;
		pcl.m_s = m_s;
...
int output_model_to_hdf5_file(Model_T2D_CHM_p& md, ResultFile_hdf5& rf);

// output the particle data and material models to hdf5 (used by time history)
int time_history_complete_output_to_hdf5_file(Model_T2D_CHM_p& md, ResultFile_hdf5& rf, hid_t frame_grp_id);

// load model data from hdf5 to model data
int load_CHM_s_model_from_hdf5_file(Model_T2D_CHM_p& md, const char* hdf5_name, const char* th_name, size_t frame_id);

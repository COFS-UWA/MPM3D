...
int TimeHistory_T2D_CHM_s_ud_complete::init()
{
	if (is_init)
		return 0;
...
void TimeHistory_T2D_CHM_s_ud_complete::close()
{
	const char* res_file_type = res_file->get_type();
	if (!strcmp(res_file_type, "ResultFile_XML"))
	{
...
{
	TimeHistory_T2D_CHM_s_ud_complete &th = static_cast<TimeHistory_T2D_CHM_s_ud_complete &>(_self);
	Step_T2D_CHM_s& step = static_cast<Step_T2D_CHM_s&>(th.get_step());
	Model_T2D_CHM_s& model	= static_cast<Model_T2D_CHM_s&>(step.get_model());

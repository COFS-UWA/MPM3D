...
int TimeHistory_T2D_CHM_s_complete::init()
{
	if (is_init)
		return 0;
...
void TimeHistory_T2D_CHM_s_complete::close()
{
	const char* res_file_type = res_file->get_type();
	if (!strcmp(res_file_type, "ResultFile_XML"))
	{
...
{
	TimeHistory_T2D_CHM_s_complete &th = static_cast<TimeHistory_T2D_CHM_s_complete &>(_self);

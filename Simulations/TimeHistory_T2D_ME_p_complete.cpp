...
int TimeHistory_T2D_ME_p_complete::init()
{
	if (is_init)
		return 0;
...
void TimeHistory_T2D_ME_p_complete::close()
{
	const char* res_file_type = res_file->get_type();
	if (!strcmp(res_file_type, "ResultFile_XML"))
	{
...
{
	TimeHistory_T2D_ME_p_complete &th = static_cast<TimeHistory_T2D_ME_p_complete &>(_self);
	Step_T2D_ME_p& step = static_cast<Step_T2D_ME_p&>(th.get_step());
	Model_T2D_ME_p& model = static_cast<Model_T2D_ME_p&>(step.get_model());

...
int Model_T2D_ME_s::apply_rigid_rect(double dt)
{
	rigid_rect.reset_f_contact();
	...
	{
		Particle& pcl = pcls[p_id];
		if (pcl.pe && rigid_rect.detect_collision_with_point(
			pcl.x, pcl.y, pcl.vol, dist, norm_x, norm_y))
		{
			f_cont = K_cont * dist;
			fx_cont = f_cont * norm_x;
			fy_cont = f_cont * norm_y;
			// add reaction force to rigid object
			rigid_rect.add_f_contact(pcl.x, pcl.y, -fx_cont, -fy_cont);
			// adjust velocity at nodes
			Element& e = *pcl.pe;
			// node 1
			Node& n1 = nodes[e.n1];

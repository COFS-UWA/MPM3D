...
#include "Model_T2D_CHM_p_hdf5_utilities.h"

#include "TimeHistory_T2D_CHM_p_complete.h"

int TimeHistory_T2D_CHM_p_complete::init()
{
	if (is_init)
		return 0;
...
void TimeHistory_T2D_CHM_p_complete::close()
{
	const char* res_file_type = res_file->get_type();
	if (!strcmp(res_file_type, "ResultFile_XML"))
	{
...
{
	TimeHistory_T2D_CHM_p_complete &th = static_cast<TimeHistory_T2D_CHM_p_complete &>(_self);
	Step_T2D_CHM_p& step = static_cast<Step_T2D_CHM_p&>(th.get_step());
	Model_T2D_CHM_p& model	= static_cast<Model_T2D_CHM_p&>(step.get_model());
...
#define str_buffer_len (sizeof(str_buffer) / sizeof(str_buffer[0]))
	...
	Model_T2D_CHM_p::Particle* pcls = model.get_pcls();
	...
	{
		Model_T2D_CHM_p::Particle &pcl = pcls[pcl_id];
		file << "        ";

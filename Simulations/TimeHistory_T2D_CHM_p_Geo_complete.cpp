...
#include "TimeHistory_T2D_CHM_p_Geo_complete.h"

int TimeHistory_T2D_CHM_p_Geo_complete::init()
{
	if (is_init)
		return 0;
...
void TimeHistory_T2D_CHM_p_Geo_complete::close()
{
	const char* res_file_type = res_file->get_type();
	if (!strcmp(res_file_type, "ResultFile_XML"))
	{
...
int time_history_output_func_t2d_chm_p_geo_complete_to_xml_res_file(TimeHistory &_self)
{
	...
	Model_T2D_CHM_p& model = static_cast<Model_T2D_CHM_p&>(th.get_model());
...
void TimeHistory_T2D_CHM_p_Geo_complete::finalize_per_step()
{

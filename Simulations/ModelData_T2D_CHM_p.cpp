...
#include "ModelData_T2D_CHM_p.h"

int model_data_output_func_t2d_chm_p_to_xml_res_file(ModelData &_self)
{
	ModelData_T2D_CHM_p &md = static_cast<ModelData_T2D_CHM_p &>(_self);
	Model_T2D_CHM_p &model = static_cast<Model_T2D_CHM_p &>(*md.model);
	...
	Model_T2D_CHM_p::Node *nodes = model.get_nodes();
	size_t elem_num = model.get_elem_num();
	Model_T2D_CHM_p::Element* elems = model.get_elems();
...
int model_data_output_func_t2d_chm_p_to_hdf5_res_file(ModelData &_self)
{
	...
	using Model_T2D_CHM_p_hdf5_utilities::output_model_to_hdf5_file;

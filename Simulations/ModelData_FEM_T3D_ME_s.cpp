...
{
	ModelData_FEM_T3D_ME_s &md = static_cast<ModelData_FEM_T3D_ME_s &>(_self);
	Model_FEM_T3D_ME_s &model = static_cast<Model_FEM_T3D_ME_s &>(*md.model);
	...
	Model_FEM_T3D_ME_s::Node *nodes = model.get_nodes();
	size_t elem_num = model.get_elem_num();
	Model_FEM_T3D_ME_s::Element *elems = model.get_elems();
...
int model_data_output_func_fem_t3d_me_s_to_hdf5_res_file(ModelData &_self)
{
	...
	Model_FEM_T3D_ME_s_hdf5_utilities::output_model_to_hdf5_file(model, rf);
	return 0;
}

	...
	)
{
	...
	Model_T2D_ME_p::Node* nodes = md.get_nodes();
	size_t elem_num = md.get_elem_num();
	Model_T2D_ME_p::Element* elems = md.get_elems();
	...
	for (size_t n_id = 0; n_id < node_num; ++n_id)
	{
		...
		Model_T2D_ME_p::Node n = nodes[n_id];
		node_data.id = n.id;
		node_data.x = n.x;
		node_data.y = n.y;
	}
	...
	for (size_t e_id = 0; e_id < elem_num; ++e_id)
	{
		...
		Model_T2D_ME_p::Element &e = elems[e_id];
		elem_data.id = e.id;
		elem_data.n1 = e.n1;
		elem_data.n2 = e.n2;
		elem_data.n3 = e.n3;
	...
	)
{
	...
	{
		Elem2DData &elem_data = elems_data[e_id];
		Model_T2D_ME_p::Element &elem = elems[e_id];
	...
	)
{
	...
	Model_T2D_ME_p::Particle* pcls = md.get_pcls();
	...
	for (size_t p_id = 0; p_id < md.pcl_num; ++p_id)
	{
		...
		Model_T2D_ME_p::Particle& pcl = md.pcls[p_id];
		pd.from_pcl(pcl);
	}
	...
	)
{
	...
	if (mm_num)
	{
		...
		{
			mm_data[mm_id].pcl_id = get_pcl_id<Model_T2D_ME_p::Particle>(iter->ext_data);

...
void test_t2d_me_s_pipe_conference_geo_undrained(int argc, char** argv)
{
	...
	for (size_t p_id = 0; p_id < pcl_num; ++p_id)
	{
		...
		mm.set_param_NC(0.3, 0.044, 0.205, 23.5, 3.6677, ini_stress, 0.0);
	...
	res_file_hdf5.create("t2d_me_s_pipe_conference_geo_undrained.h5");
...
{
	ResultFile_hdf5 rf;
	rf.open("t2d_me_s_pipe_conference_geo_undrained.h5");
	...
	//app.set_png_name("t2d_me_s_pipe_conference_geo_undrained");
	app.start();
	...
	//app.set_color_map_pos(0.8, 0.65, 0.3);
	////app.set_png_name("t2d_me_s_pipe_conference_geo_undrained");
	////app.set_gif_name("t2d_me_s_pipe_conference_geo_undrained");
	//app.start();
}

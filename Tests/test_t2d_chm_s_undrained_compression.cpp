...
void test_t2d_chm_s_ud_compression(int argc, char** argv)
{
	...
	model.init_pcls(pcl_generator, 0.4, 20.0, 10.0, 400.0, 1.0e-4, 1.0);
	...
	res_file_hdf5.create("t2d_chm_s_ud_compression.h5");
	
	ModelData_T2D_CHM_s md;
	md.output_model(model, res_file_hdf5);

	TimeHistory_T2D_CHM_s_ud_complete out1("compression");
...
{
	ResultFile_hdf5 rf;
	rf.open("t2d_chm_s_ud_compression.h5");

	QtApp_Posp_T2D_CHM_s app(argc, argv, QtApp_Posp_T2D_CHM_s::Animation);
	app.set_win_size(900, 900);
	app.set_res_file(rf, "compression", Hdf5Field::p);
	app.set_ani_time(5.0);
	app.set_color_map_fld_range(-10.0, 10.0);
	app.set_color_map_geometry(0.75, 0.5, 0.4);
	//app.set_png_name("t2d_chm_s_ud_compression");
	//app.set_gif_name("t2d_chm_s_ud_compression");
	app.start();
}

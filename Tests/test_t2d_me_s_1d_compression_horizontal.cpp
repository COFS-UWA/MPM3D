...
void test_t2d_me_s_1d_compression_horizontal(int argc, char **argv)
{
	...
	model.load_mesh_from_hdf5("../../Asset/rect_mesh_horizonal.h5");
	model.init_search_grid(0.05, 0.05);

	ParticleGenerator2D<Model_T2D_ME_s> pcl_generator;
	pcl_generator.generate_pcls_in_grid_layout(Rect(0.0, 1.0, 0.0, 0.2), 0.02, 0.02);
	...
	find_2d_nodes_on_y_line(model, vy_bc_pt_array, 0.2, false);
	...
	find_2d_pcls(model, tbc_pt_array, Rect(0.987, 1.0, 0.0, 0.2));
	...
	res_file_hdf5.create("t2d_me_s_1d_compression_horizontal.h5");

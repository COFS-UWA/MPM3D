...
struct ParticleData
{
	...
	void from_pcl(
		Step_T2D_CHM_TBB &stp,
		size_t pcl_offset)
	{
		Step_T2D_CHM_Task::CalData& cd = stp.cal_data;
		auto& spva = cd.spvas[cd.sorted_pcl_var_id];
		id = spva.pcl_index[pcl_offset];
		const Model_T2D_CHM_mt::Force& pcl_bf_s = cd.pcl_bf_s[id];
		bfx_s = pcl_bf_s.fx;
		bfy_s = pcl_bf_s.fy;
		const Model_T2D_CHM_mt::Force& pcl_bf_f = cd.pcl_bf_f[id];
		bfx_f = pcl_bf_f.fx;
		bfy_f = pcl_bf_f.fy;
		const Model_T2D_CHM_mt::Force& pcl_t = cd.pcl_t[id];
		tx = pcl_t.fx;
		ty = pcl_t.fy;
		...
		vol = m_s / (density_s * (1.0 - n));
		const Model_T2D_CHM_mt::Position& pcl_pos = cd.pcl_pos[id];
		const Model_T2D_CHM_mt::Displacement& pcl_u_s = spva.pcl_u_s[pcl_offset];
		x = pcl_pos.x + pcl_u_s.ux;
		y = pcl_pos.y + pcl_u_s.uy;
		Model_T2D_CHM_mt::Displacement& pcl_u_f = spva.pcl_u_f[pcl_offset];
		ux_f = pcl_u_f.ux - pcl_u_s.ux;
		uy_f = pcl_u_f.uy - pcl_u_s.uy;
		Model_T2D_CHM_mt::Velocity& pcl_v_s = spva.pcl_v_s[pcl_offset];
		vx_s = pcl_v_s.vx;
		vy_s = pcl_v_s.vy;
		Model_T2D_CHM_mt::Velocity& pcl_v_f = spva.pcl_v_f[pcl_offset];
		vx_f = pcl_v_f.vx;
		vy_f = pcl_v_f.vy;
		Model_T2D_CHM_mt::Stress& pcl_stress = spva.pcl_stress[pcl_offset];
		s11 = pcl_stress.s11;
		s22 = pcl_stress.s22;
		s12 = pcl_stress.s12;
		p = spva.pcl_p[pcl_offset];
		Model_T2D_CHM_mt::Strain& pcl_e = spva.pcl_strain[pcl_offset];
		e11 = pcl_e.e11;
		e22 = pcl_e.e22;
		e12 = pcl_e.e12;
		Model_T2D_CHM_mt::Strain& pcl_ee = spva.pcl_estrain[pcl_offset];
		ee11 = pcl_ee.e11;
		ee22 = pcl_ee.e22;
		ee12 = pcl_ee.e12;
		Model_T2D_CHM_mt::Strain& pcl_pe = spva.pcl_pstrain[pcl_offset];
		pe11 = pcl_pe.e11;
		pe22 = pcl_pe.e22;
		pe12 = pcl_pe.e12;
...
int output_pcl_data_to_hdf5_file(Model_T2D_CHM_mt& md, Step_T2D_CHM_TBB& stp, ResultFile_hdf5& rf, hid_t grp_id);
int output_pcl_data_to_hdf5_file(Model_T2D_CHM_mt& md, Step_T2D_CHM_mt_Geo& stp, ResultFile_hdf5& rf, hid_t grp_id);
...
int time_history_complete_output_to_hdf5_file(Model_T2D_CHM_mt& md, Step_T2D_CHM_TBB& stp, ResultFile_hdf5& rf, hid_t frame_grp_id);
int time_history_complete_output_to_hdf5_file(Model_T2D_CHM_mt& md, Step_T2D_CHM_mt_Geo& stp, ResultFile_hdf5& rf, hid_t frame_grp_id);

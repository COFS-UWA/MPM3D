...
namespace SortUtils
{
	...
	tbb::task* SortParticleTask::execute()
	{
		...
		const size_t pcl_digit_pos = pcl_digit_num - 1;
		const size_t pcl_num = valid_pcl_num;
		if (pcl_num > SortParticleMem::serial_sort_max_pcl_num)
		{
			...
			if (pcl_num > SortParticleMem::parallel_divide_min_pcl_num_per_block)
			{
				...
				{
					end_id = Block_Low(blk_id, block_num, pcl_num);
				...
				{
					SortBin& bbin = rt_bins[blk_id];
					valid_pcl_num += bbin.sum_bin[Internal::radix_bucket_num - 1]
								 + bbin.count_bin[Internal::radix_bucket_num - 1];
				}
				...
				if (pcl_digit_pos) // not the last digit
				{
					...
					for (bin_id = 0; bin_id < Internal::radix_bucket_num; ++bin_id)
					{
						...
						for (blk_id = 0; blk_id < block_num; ++blk_id)
						{
							...
							const size_t in_offset_id = Block_Low(blk_id, block_num, pcl_num) + blk_bin.sum_bin[bin_id];
			...
			else // divide serially and sort parallely
			{
				...
				Internal::count_sort(
					...
					rt_bin,
					pcl_digit_pos);
				valid_pcl_num = rt_bin.sum_bin[Internal::radix_bucket_num - 1]
							+ rt_bin.count_bin[Internal::radix_bucket_num - 1];
				if (pcl_digit_pos) // not the last digit
				{
		...
		sort_mem.pcl_in_elem[valid_pcl_num] = SIZE_MAX;
		return nullptr;
	}
}

...
{
	//ModelData_T2D_CHM_mt &md = static_cast<ModelData_T2D_CHM_mt &>(_self);
	//Model_T2D_CHM_mt &model = static_cast<Model_T2D_CHM_mt &>(*md.model);
	...
	//Model_T2D_CHM_mt::Node* nodes = model.get_nodes();
	//size_t elem_num = model.get_elem_num();
	//Model_T2D_CHM_mt::Element* elems = model.get_elems();
	...
	//{
	//	Model_T2D_CHM_mt::Node &n = nodes[n_id];
	...
	//{
	//	Model_T2D_CHM_mt::Element &e = elems[e_id];
...
int model_data_output_func_t2d_chm_mt_to_hdf5_res_file(ModelData &_self)
{
	...
	using Model_T2D_CHM_mt_hdf5_utilities::output_model_to_hdf5_file;

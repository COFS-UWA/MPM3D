...
namespace Step_T2D_CHM_Task
{
	...
	void MapPclToBgMesh::operator() (size_t wk_id) const
	{
		...
		for (size_t p_id = p_id0; p_id < p_id1; ++p_id)
		{
			...
			const double one_third_bfx_s = one_third * p_bf_s.fx;
			const double one_third_bfy_s = one_third * p_bf_s.fy;
			...
			const double one_third_bfx_f = one_third * p_bf_f.fx;
			const double one_third_bfy_f = one_third * p_bf_f.fy;
			...
			if (e_id != pcl_in_elem[p_id + 1])
			{
				...
				{
					const double vol_ratio = elem_area[e_id] / e_p_vol;
...
#endif
			n_am_s += elem_pcl_m_s[ne_id / 3];
			n_am_f += elem_pcl_m_f[ne_id / 3];
	...
	void CalElemDeAndMapToNode::operator() (size_t wk_id) const
	{
		...
		for (size_t ve_id = ve_id0; ve_id < ve_id1; ++ve_id)
		{
			...
			e_de.de11 = (e_dN.dN1_dx * n1_v_s.vx + e_dN.dN2_dx * n2_v_s.vx + e_dN.dN3_dx * n3_v_s.vx) * cd.dt;
			e_de.de22 = (e_dN.dN1_dy * n1_v_s.vy + e_dN.dN2_dy * n2_v_s.vy + e_dN.dN3_dy * n3_v_s.vy) * cd.dt;
			e_de.de12 = (e_dN.dN1_dx * n1_v_s.vy + e_dN.dN2_dx * n2_v_s.vy + e_dN.dN3_dx * n3_v_s.vy
					   + e_dN.dN1_dy * n1_v_s.vx + e_dN.dN2_dy * n2_v_s.vx + e_dN.dN3_dy * n3_v_s.vx) * cd.dt * 0.5;
			...
			e_de_vol_f = (1.0 - elem_pcl_n[e_id]) / elem_pcl_n[e_id] * -e_de_vol_s
					- (e_dN.dN1_dx * n1_v_f.vx + e_dN.dN2_dx * n2_v_f.vx + e_dN.dN3_dx * n3_v_f.vx
					 + e_dN.dN1_dy * n1_v_f.vy + e_dN.dN2_dy * n2_v_f.vy + e_dN.dN3_dy * n3_v_f.vy) * cd.dt;
	...
	void MapBgMeshToPcl::operator() (size_t wk_id, size_t& pcl_in_mesh_num) const
	{
		...
		const Acceleration* pn1_a_f, * pn2_a_f, * pn3_a_f;
		const Velocity* pn1_v_s, * pn2_v_s, * pn3_v_s;
		const Velocity* pn1_v_f, * pn2_v_f, * pn3_v_f;
		...
		for (size_t p_id = p_id0; p_id < p_id1; ++p_id)
		{
			if (e_id != pcl_in_elem[p_id])
			{
				...
				e_de_vol_f = (node_de_vol_f[eni.n1]
					...
					+ node_de_vol_f[eni.n3]) * one_third;
			...
			Velocity& p_v_s0 = pcl_v_s0[p_id];
			p_v_s0.vx += (p_N.N1 * pn1_a_s->ax + p_N.N2 * pn2_a_s->ax + p_N.N3 * pn3_a_s->ax) * cd.dt;
			p_v_s0.vy += (p_N.N1 * pn1_a_s->ay + p_N.N2 * pn2_a_s->ay + p_N.N3 * pn3_a_s->ay) * cd.dt;
			Velocity& p_v_f0 = pcl_v_f0[p_id];
			p_v_f0.vx += (p_N.N1 * pn1_a_f->ax + p_N.N2 * pn2_a_f->ax + p_N.N3 * pn3_a_f->ax) * cd.dt;
			p_v_f0.vy += (p_N.N1 * pn1_a_f->ay + p_N.N2 * pn2_a_f->ay + p_N.N3 * pn3_a_f->ay) * cd.dt;

			// update displacement
			Displacement& p_u_s0 = pcl_u_s0[p_id];
			p_u_s0.ux += (p_N.N1 * pn1_v_s->vx + p_N.N2 * pn2_v_s->vx + p_N.N3 * pn3_v_s->vx) * cd.dt;
			p_u_s0.uy += (p_N.N1 * pn1_v_s->vy + p_N.N2 * pn2_v_s->vy + p_N.N3 * pn3_v_s->vy) * cd.dt;
			Displacement& p_u_f0 = pcl_u_f0[p_id];
			p_u_f0.ux += (p_N.N1 * pn1_v_f->vx + p_N.N2 * pn2_v_f->vx + p_N.N3 * pn3_v_f->vx) * cd.dt;
			p_u_f0.uy += (p_N.N1 * pn1_v_f->vy + p_N.N2 * pn2_v_f->vy + p_N.N3 * pn3_v_f->vy) * cd.dt;

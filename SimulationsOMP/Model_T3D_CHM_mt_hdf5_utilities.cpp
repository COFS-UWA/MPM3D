	...
	hid_t grp_id)
{
	...
	Model_T3D_CHM_mt::Position *node_pos = md.node_pos;
	for (size_t n_id = 0; n_id < node_num; ++n_id)
	{
		NodeData &node_data = nodes_data[n_id];
		node_data.id = n_id;
		Model_T3D_CHM_mt::Position &np = node_pos[n_id];
		node_data.x = np.x;
		node_data.y = np.y;
		node_data.z = np.z;
	}
	...
	Model_T3D_CHM_mt::ElemNodeIndex *e_node_index = md.elem_node_id;
	double* e_vol = md.elem_vol;
	Model_T3D_CHM_mt::DShapeFuncABC* e_sf_abc = md.elem_N_abc;
	Model_T3D_CHM_mt::DShapeFuncD* e_sf_d = md.elem_N_d;
	for (size_t e_id = 0; e_id < elem_num; ++e_id)
	{
		ElementData &elem_data = elems_data[e_id];
		elem_data.id = e_id;
		Model_T3D_CHM_mt::ElemNodeIndex &eni = e_node_index[e_id];
		elem_data.n1 = eni.n1;
		elem_data.n2 = eni.n2;
		elem_data.n3 = eni.n3;
		elem_data.n4 = eni.n4;
		elem_data.vol = e_vol[e_id];
		Model_T3D_CHM_mt::DShapeFuncABC &esfabc = e_sf_abc[e_id];
		...
		Model_T3D_CHM_mt::DShapeFuncD& esfd = e_sf_d[e_id];
	...
	hid_t grp_id)
{
	...
	rf.read_dataset(
		...
		ed_dt_id);
	H5Tclose(ed_dt_id);
	Model_T3D_CHM_mt::ElemNodeIndex* e_node_id = md.elem_node_id;
	double* e_vol = md.elem_vol;
	Model_T3D_CHM_mt::DShapeFuncABC* e_sf_abc = md.elem_N_abc;
	Model_T3D_CHM_mt::DShapeFuncD* e_sf_d = md.elem_N_d;
	for (size_t e_id = 0; e_id < elem_num; ++e_id)
	{
		...
		Model_T3D_CHM_mt::ElemNodeIndex &eni = e_node_id[e_id];
		eni.n1 = elem_data.n1;
		eni.n2 = elem_data.n2;
		eni.n3 = elem_data.n3;
		eni.n4 = elem_data.n4;
	...
	hid_t grp_id)
{
	...
	for (size_t n_id = 0; n_id < md.node_num; ++n_id)
	{
		...
		Model_T3D_CHM_mt::NodeHasVBC& md_nvbc = md.node_has_vbc_s[n_id];
	...
	{
		NodeVBCData& nvbc = nvbc_data[n_id];
		Model_T3D_CHM_mt::NodeHasVBC& md_nvbc = md.node_has_vbc_f[n_id];
		nvbc.has_vx_bc = md_nvbc.has_vx_bc;
		nvbc.has_vy_bc = md_nvbc.has_vy_bc;
		nvbc.has_vz_bc = md_nvbc.has_vz_bc;
	}
	...
	for (size_t n_id = 0; n_id < md.node_num; ++n_id)
	{
		...
		const Model_T3D_CHM_mt::NodeVBCVec& md_nvbc = md.node_vbc_vec_s[n_id];
		nvbc.x = md_nvbc.x;
		nvbc.y = md_nvbc.y;
		nvbc.z = md_nvbc.z;
	}
	rf.write_dataset(bc_grp_id, "NodeVBCVector_s", md.node_num, nvbc_vec, n_vbc_vec_dt_id);
	for (size_t n_id = 0; n_id < md.node_num; ++n_id)
	{
		NodeVBCVecData& nvbc = nvbc_vec[n_id];
		const Model_T3D_CHM_mt::NodeVBCVec& md_nvbc = md.node_vbc_vec_f[n_id];
		nvbc.x = md_nvbc.x;
		nvbc.y = md_nvbc.y;
		nvbc.z = md_nvbc.z;
	}
	rf.write_dataset(bc_grp_id, "NodeVBCVector_f", md.node_num, nvbc_vec, n_vbc_vec_dt_id);
	delete[] nvbc_vec;
	...
	hid_t grp_id)
{
	...
	{
		NodeVBCData& nvbc = nvbc_data[n_id];
		Model_T3D_CHM_mt::NodeHasVBC &md_nvbc
	...
	hid_t n_vbc_vec_dt_id = get_node_vbc_vec_dt_id();
	NodeVBCVecData* nvbc_vec = new NodeVBCVecData[md.node_num];
	rf.read_dataset(bc_id, "NodeVBCVector_s", md.node_num, nvbc_vec, n_vbc_vec_dt_id);
	for (size_t n_id = 0; n_id < md.node_num; ++n_id)
	{
		NodeVBCVecData& nvbc = nvbc_vec[n_id];
		Model_T3D_CHM_mt::NodeVBCVec& md_nvbc = md.node_vbc_vec_s[n_id];
		md_nvbc.x = nvbc.x;
		md_nvbc.y = nvbc.y;
		md_nvbc.z = nvbc.z;
	}
	rf.read_dataset(bc_id, "NodeVBCVector_f", md.node_num, nvbc_vec, n_vbc_vec_dt_id);
	for (size_t n_id = 0; n_id < md.node_num; ++n_id)
	{
		NodeVBCVecData& nvbc = nvbc_vec[n_id];
		Model_T3D_CHM_mt::NodeVBCVec& md_nvbc = md.node_vbc_vec_f[n_id];
		md_nvbc.x = nvbc.x;
		md_nvbc.y = nvbc.y;
		md_nvbc.z = nvbc.z;
	}
	...
	hid_t grp_id)
{
	...
	if (pcl_num)
	{
		ParticleData* pcl_data = new ParticleData[pcl_num];
		for (size_t p_id = 0; p_id < pcl_num; ++p_id)
			pcl_data[p_id].from_pcl(md, p_id, 0);
		hid_t pcl_dt_id = get_pcl_dt_id();
		res = rf.write_dataset(
			pcl_data_grp_id,
			"field",
...
int output_pcl_data_to_hdf5_file(
	Model_T3D_CHM_mt& md,
	Step_T3D_CHM_ud_mt_subiter& stp,
	ResultFile_hdf5& rf,
	hid_t grp_id)
{
	if (grp_id < 0)

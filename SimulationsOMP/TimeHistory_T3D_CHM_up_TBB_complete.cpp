...
int TimeHistory_T3D_CHM_up_TBB_complete::init()
{
	if (is_init)
		return 0;
...
void TimeHistory_T3D_CHM_up_TBB_complete::close()
{
	const char* res_file_type = res_file->get_type();
	if (!strcmp(res_file_type, "ResultFile_XML"))
	{
...
{
//	TimeHistory_T3D_CHM_up_TBB_complete &th = static_cast<TimeHistory_T3D_CHM_up_TBB_complete &>(_self);
...
{
	TimeHistory_T3D_CHM_up_TBB_complete &th = static_cast<TimeHistory_T3D_CHM_up_TBB_complete &>(_self);
	Step_T3D_CHM_up_TBB &step = static_cast<Step_T3D_CHM_up_TBB &>(th.get_step());

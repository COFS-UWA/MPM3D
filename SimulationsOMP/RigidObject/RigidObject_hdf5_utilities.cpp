...
namespace RigidObject_hdf5_utilities
{
		...
		hid_t rc_grp_id)
	{
		...
		const Vector3D& v_bc = rc.get_velocity_bc();
		rf.write_attribute(rc_grp_id, "vx_bc", v_bc.x);
		rf.write_attribute(rc_grp_id, "vy_bc", v_bc.y);
		rf.write_attribute(rc_grp_id, "vz_bc", v_bc.z);
		rf.write_attribute(rc_grp_id, "vx_bc_mask", rc.get_vx_bc_mask());
		rf.write_attribute(rc_grp_id, "vy_bc_mask", rc.get_vy_bc_mask());
		rf.write_attribute(rc_grp_id, "vz_bc_mask", rc.get_vz_bc_mask());
		...
		hid_t rc_grp_id)
	{
		...
		rf.read_attribute(rc_grp_id, "density", den);
		...
		rf.read_attribute(rc_grp_id, "mz_ext", mz_ext);
		rc.init(x, y, z, h, r, den);
		rc.set_init_v(vx, vy, vz);
		
		size_t bc_mask;
		double v_bc;
		rf.read_attribute(rc_grp_id, "vx_bc_mask", bc_mask);
		if (bc_mask)
		{
			rf.read_attribute(rc_grp_id, "vx_bc", v_bc);
			rc.set_vx_bc(v_bc);
		}
		rf.read_attribute(rc_grp_id, "vy_bc_mask", bc_mask);
		if (bc_mask)
		{
			rf.read_attribute(rc_grp_id, "vy_bc", v_bc);
			rc.set_vy_bc(v_bc);
		}
		rf.read_attribute(rc_grp_id, "vz_bc_mask", bc_mask);
		if (bc_mask)
		{
			rf.read_attribute(rc_grp_id, "vz_bc", v_bc);
			rc.set_vz_bc(v_bc);
		}
	...
	int load_rigid_object_by_3dmesh_state_from_hdf5_file(RigidObjectByT3DMesh& rb, ResultFile_hdf5& rf, hid_t rb_grp_id)
	{
		union
		{
			struct { double rb_x, rb_y, rb_z; };
			struct { double rb_x_ang, rb_y_ang, rb_z_ang; };
			struct { double rb_vx, rb_vy, rb_vz; };
			struct { double rb_vx_ang, rb_vy_ang, rb_vz_ang; };
		};
		//double rb_x_ang, rb_y_ang, rb_z_ang;
		rf.read_attribute(rb_grp_id, "x", rb_x);
		rf.read_attribute(rb_grp_id, "y", rb_y);
		rf.read_attribute(rb_grp_id, "z", rb_z);
		rb.set_position(rb_x, rb_y, rb_z);
		
		rf.read_attribute(rb_grp_id, "x_ang", rb_x_ang);
		rf.read_attribute(rb_grp_id, "y_ang", rb_y_ang);
		rf.read_attribute(rb_grp_id, "z_ang", rb_z_ang);
		rb.set_pos_ang(rb_x_ang, rb_y_ang, rb_z_ang);
		
		rf.read_attribute(rb_grp_id, "vx", rb_vx);
		rf.read_attribute(rb_grp_id, "vy", rb_vy);
		rf.read_attribute(rb_grp_id, "vz", rb_vz);
		rb.set_velocity(rb_vx, rb_vy, rb_vz);
		
		rf.read_attribute(rb_grp_id, "vx_ang", rb_vx_ang);

...
Step_T3D_ME_mt_Geo::Step_T3D_ME_mt_Geo(const char* _name) :
	...
	f_ub_ratio_limit(0.01), e_kin_ratio_limit(0.01) {}
...
{
#ifdef _DEBUG
	res_file_t3d_me_mt_geo.open("t3d_stp_mt_geo.txt", std::ios::out | std::ios::binary);
#endif
...
#pragma omp parallel
	{
		...
		size_t* pcl_in_elem0 = pcl_in_elems[1];
		size_t* prev_pcl_id0 = prev_pcl_ids[1];
...
#pragma omp critical
		...
		size_t* pcl_in_elem1 = pcl_in_elems[0];
		size_t* prev_pcl_id1 = prev_pcl_ids[0];
...
#pragma omp barrier
			...
			for (p_id = p_id1; p_id-- > p_id0;)
			{
				...
				assert((pcl_in_elem0[p_id] < elem_num ||
					pcl_in_elem0[p_id] == SIZE_MAX) &&
					(prev_pcl_id0[p_id] < prev_valid_pcl_num));
			}
#pragma omp barrier
		}
...
#pragma omp barrier
	...
	size_t* const my_node_has_elem = self.node_has_elems[0] + e_id * 4;
	size_t* const my_node_elem_pair = self.node_elem_pairs[0] + e_id * 4;
	size_t my_valid_elem_num = 0;
	for (p_id = p_id0; p_id < p_id1; ++p_id)
	{
		// pcl index
...
#pragma omp single
	{
		...
		{
			f_ub_ratio = sqrt(self.f_ub / self.init_f_ub);
		}
		else
		{
		...
		if (self.e_kin_max_is_init)
			e_kin_ratio = sqrt(self.e_kin_prev / self.e_kin_max);
		else
			e_kin_ratio = 1.0;
		...
		{
			if (f_ub_ratio < self.f_ub_ratio_limit &&
				e_kin_ratio < self.e_kin_ratio_limit)
			{
				// exit
				self.cal_status = 2;
			}
...
#pragma omp barrier
	...
	for (p_id = p_id0; p_id < p_id1; ++p_id)
	{
		if (e_id != pcl_in_elem0[p_id])
		{
			...
			e_de_vol = one_fourth * (node_de_vol[eni.n1] + node_de_vol[eni.n2]

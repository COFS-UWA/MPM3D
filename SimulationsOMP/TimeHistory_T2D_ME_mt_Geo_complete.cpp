...
int TimeHistory_T2D_ME_mt_Geo_complete::init()
{
	if (is_init)
		return 0;
...
void TimeHistory_T2D_ME_mt_Geo_complete::close()
{
	const char* res_file_type = res_file->get_type();
	if (!strcmp(res_file_type, "ResultFile_XML"))
	{
...
{
//	TimeHistory_T2D_ME_mt_Geo_complete &th = static_cast<TimeHistory_T2D_ME_mt_Geo_complete &>(_self);
...
{
	TimeHistory_T2D_ME_mt_Geo_complete &th = static_cast<TimeHistory_T2D_ME_mt_Geo_complete &>(_self);
	Step_T2D_ME_mt_Geo &step = static_cast<Step_T2D_ME_mt_Geo &>(th.get_step());

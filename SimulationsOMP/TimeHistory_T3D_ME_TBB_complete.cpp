...
int TimeHistory_T3D_ME_TBB_complete::init()
{
	if (is_init)
		return 0;
...
void TimeHistory_T3D_ME_TBB_complete::close()
{
	const char* res_file_type = res_file->get_type();
	if (!strcmp(res_file_type, "ResultFile_XML"))
	{
...
{
//	TimeHistory_T3D_ME_TBB_complete &th = static_cast<TimeHistory_T3D_ME_TBB_complete &>(_self);
...
{
	TimeHistory_T3D_ME_TBB_complete &th = static_cast<TimeHistory_T3D_ME_TBB_complete &>(_self);
	Step_T3D_ME_TBB &step = static_cast<Step_T3D_ME_TBB &>(th.get_step());
	...
	Model_T3D_ME_mt_hdf5_utilities::time_history_complete_output_to_hdf5_file(md, step, rf, frame_grp_id);

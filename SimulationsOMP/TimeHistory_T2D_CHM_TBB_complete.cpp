...
#include "TimeHistory_T2D_CHM_TBB_complete.h"

int TimeHistory_T2D_CHM_TBB_complete::init()
{
	if (is_init)
		return 0;
...
void TimeHistory_T2D_CHM_TBB_complete::close()
{
	const char* res_file_type = res_file->get_type();
	if (!strcmp(res_file_type, "ResultFile_XML"))
	{
...
{
//	TimeHistory_T2D_CHM_TBB_complete &th = static_cast<TimeHistory_T2D_CHM_TBB_complete &>(_self);
//	Step_T2D_CHM_TBB& step = static_cast<Step_T2D_CHM_TBB&>(th.get_step());
//	Model_T2D_CHM_mt& model = static_cast<Model_T2D_CHM_mt&>(step.get_model());
...
//	Model_T2D_CHM_mt::Particle *pcls = model.get_pcls();
...
//	{
//		Model_T2D_CHM_mt::Particle &pcl = pcls[pcl_id];
//		file << "        ";
...
int time_history_output_func_T2D_CHM_TBB_to_hdf5_res_file(TimeHistory &_self)
{
	...
	using Model_T2D_CHM_mt_hdf5_utilities::time_history_complete_output_to_hdf5_file;

...
csv_file = open("../Build/TestsParallel/t3d_me_mt_1d_compression_s33.csv", "w")
...
s33_ana = np.zeros(data_num)
t_inv = t_len / float(data_num)
for i in range(data_num):
    t_ana[i] = t_inv * i
    s33_ana[i] = bav.stress(init_z, t_ana[i])

csv_file.write("analytical,\n")
for o_id in range(data_num):
    csv_file.write("%f, %f\n" % (t_ana[o_id], s33_ana[o_id]))
...
line2, = plot1.plot(t_ana, s33_ana)
